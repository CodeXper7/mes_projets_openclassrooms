	...
	def show(self, window):
		...
		for line in self.structure:
			...
			for sprite in line:
				...
				num_step += 1
			num_line += 1
		three_xy_of_objetcs = [xy_objetcs[self.r_list[n]] for n in range(0, 3)] #selection les trois paires x et y au azar

		self.three_xy_of_objetcs = three_xy_of_objetcs
		#list to add the three objects randomly
		self.paste_the_3_objects.append([window.blit(syringe[i], (three_xy_of_objetcs[i][0], three_xy_of_objetcs[i][1])) for i in range(3)])

		for x in self.mgpaths:

			if x in self.three_xy_of_objetcs:
				window.blit(erase_object_image, (x[0], x[1]))
				# variable to paste texts on the screen
